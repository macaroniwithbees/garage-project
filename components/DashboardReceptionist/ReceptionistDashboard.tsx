...
import type { AppointmentRow, InvoiceRow, MechanicOption, ReceptionistAppointment, RepairRow, UserRow } from "./types";
...
export default function ReceptionistDashboard() {
	...
	useEffect(() => {
		const loadDashboardData = async () => {
			...
			const [appointmentsResponse, usersResponse, repairsResponse, invoicesResponse] = await Promise.all([
			...
			if (appointmentsResponse.error) {
				setErrorText(`Kon afspraken niet laden: ${appointmentsResponse.error.message}`);
				setLoading(false);
				return;
			}
			if (usersResponse.error) {
				setErrorText(`Kon gebruikers niet laden: ${usersResponse.error.message}`);
				setLoading(false);
				return;
			}
			...
			const userById = new Map<number, UserRow>(userRows.map((user) => [user.id, user]));
			...
			for (const repair of repairRows) {
				if (!serviceByAppointmentId.has(repair.appointment_id) && repair.beschrijving) {
					const repairParts = repair.beschrijving.split("|").map((part) => part.trim());
			...
			setMechanics(
				userRows
					...
					.map((user) => ({ id: user.id, naam: user.naam ?? user.email ?? `Monteur #${user.id}` })),
			);

			if (currentUser?.email) {
				const receptionist = userRows.find((user) => user.email === currentUser.email);
				setProfileName(
					receptionist?.naam ?? currentUser.user_metadata?.full_name ?? currentUser.email.split("@")[0] ?? "Receptionist",
				);
			}

			const mappedAppointments = appointmentRows.map((appointment) => {
				const customer = appointment.user_id ? userById.get(appointment.user_id) : undefined;
				const mechanic = appointment.toegewezen_monteur ? userById.get(appointment.toegewezen_monteur) : undefined;
				const invoice = invoiceByAppointmentId.get(appointment.id);

				return {
					id: appointment.id,
					voertuig: toVehicleLabel(appointment),
					klant: customer?.naam ?? customer?.email ?? `Klant #${appointment.user_id ?? "-"}`,
					...
					opmerkingen: appointment.opmerkingen ?? notesByAppointmentId.get(appointment.id) ?? undefined,
					monteur: mechanic?.naam ?? undefined,
					betaald: invoice ? invoice.betaald === "ja" : undefined,
	...
	const updateStatusWithFallback = async (appointmentId: number, statusCandidates: string[], mechanicId?: number) => {
		...
		for (const statusValue of statusCandidates) {
			const payload = mechanicId ? { status: statusValue, toegewezen_monteur: mechanicId } : { status: statusValue };
	...
	return (
		<div className="min-h-screen bg-slate-100 text-slate-900">
			<header className="border-b border-slate-200 bg-white px-8 py-4">
				<div className="mx-auto flex w-full max-w-[1250px] items-center justify-between">
					...
					<div className="flex items-center gap-6 text-xl">
						...
						<button className="flex items-center gap-2 text-slate-700" type="button" onClick={handleLogout}>
							<LogOut size={20} />
							Uitloggen
						</button>
					</div>
			...
			<main className="mx-auto max-w-[1250px] px-8 py-10">
				...
				<p className="mt-3 text-2xl text-slate-600">Beheer afspraken en communiceer met klanten</p>
				...
				<div className="mt-10 space-y-10">
					<section>
						<h2 className="mb-4 text-4xl font-bold">Te Bevestigen ({toConfirm.length})</h2>
					...
					<section>
						<h2 className="mb-4 text-4xl font-bold">Toe te Wijzen ({toAssign.length})</h2>
						<div className="space-y-5">
					...
					<section>
						<h2 className="mb-4 text-4xl font-bold">In Behandeling ({inProgress.length})</h2>
						<div className="space-y-5">
					...
					<section>
						<h2 className="mb-4 text-4xl font-bold">Klaar voor Ophalen ({readyForPickup.length})</h2>
						<div className="space-y-5">
							{readyForPickup.map((appointment) => (
								<AppointmentCard
									key={appointment.id}
									appointment={appointment}
									footer={appointment.betaald ? "Klant kan auto ophalen" : "Wacht op betaling van klant..."}
								/>
							))}
						</div>
					</section>
